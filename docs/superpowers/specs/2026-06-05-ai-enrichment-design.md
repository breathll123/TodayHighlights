...
今日看点已经从单一股票看板扩展为多主题信息聚合平台，当前包含股票、AI、足球等主题。现有架构具备独立 source adapter、`raw_items` 原始内容存储、`highlights` 看点展示、`page_blocks` 页面布局配置，以及后台模型设置和 Fernet 加密能力。
...
这是推荐并已确认的方案。它能体现今日看点的核心价值：从很多数据里提炼“今天值得看什么”。
